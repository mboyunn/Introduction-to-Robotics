...
if mycobot.is_power_on():
    ...
    T_sym = simplify(T_01 * T_12 * T_23 * T_34 * T_45 * T_56)
    ...
    with open("data.csv", "r", newline="") as csvfile:
        ...
        next(file)
        for row in file:
            joint_angles = [np.radians(float(x)) for x in row[0:6]]
            coords = [float(x) for x in row[6:]]

            offsets = [0, 0, 0, 0, 0, 0] # right now don't know offset yet, so just put 0 for now. later we'll need to find joins and put pi/2
            ...
            print("FK calculated coords: ", [x,y,z,0,0,0])

            mycobot.send_coords([x, y, z, 0, 0, 0], 50)
            time.sleep(0.5)
