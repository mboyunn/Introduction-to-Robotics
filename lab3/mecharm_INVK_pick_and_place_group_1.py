...
if __name__ == "__main__":
    ...
    # send to origin
    target_pose = np.array([155.2, 0.9, 219.0, np.deg2rad(-172.88), np.deg2rad(88.58), np.deg2rad(-172.52)], dtype=float)
    joint_angles = inverse_kinematics(target_pose, init_pose)
    joint_angles_deg = np.rad2deg(joint_angles)
    mycobot.send_angles(joint_angles_deg.tolist(), 50)
    init_pose = joint_angles_deg
    time.sleep(5)

    # open gripper
    mycobot.set_gripper_value(0, 50, 1)
    time.sleep(2)

    # send to location - goes down and to the right
    target_pose = np.array([170.2, 0.9, -31.0, np.deg2rad(-172.88), np.deg2rad(88.58), np.deg2rad(-172.52)], dtype=float)
    joint_angles = inverse_kinematics(target_pose, init_pose)
    joint_angles_deg = np.rad2deg(joint_angles)
    mycobot.send_angles(joint_angles_deg.tolist(), 50)
    init_pose = joint_angles_deg
    ...
    # place object - goes down
    target_pose = np.array([155.2, 0.9, -21.0, np.deg2rad(-172.88), np.deg2rad(88.58), np.deg2rad(-172.52)], dtype=float)
    joint_angles = inverse_kinematics(target_pose, init_pose)
    joint_angles_deg = np.rad2deg(joint_angles)
    mycobot.send_angles(joint_angles_deg.tolist(), 50)
    init_pose = joint_angles_deg
    ...
    # open gripper slowly
    mycobot.set_gripper_value(0, 10, 1)
    time.sleep(10)
